...
## Claude Code 作業完了自動化ルール

### 作業完了時の自動提案システム
Claude Codeでのファイル変更・実装完了時に、以下の提案が自動表示されます：

#### 1. 自動提案トリガー
- **files_modified**: ファイル変更検知時（5秒後に提案）
- **implementation_complete**: 実装完了時（即座に提案）
- **task_finished**: タスク完了時（ワークフローガイダンス表示）

#### 2. 提案メッセージ
```
📝 ファイルが変更されました。作業完了時は 'npm run dev:commit' でコミットしませんか？

✅ 実装が完了しました。変更をコミットしますか？
...
#### 3. ファイル種別別の提案
- **Ruby files (*.rb)**: 10秒後にコミットリマインダー（セッション毎1回）
- **JavaScript files (*.js)**: 10秒後にコミットリマインダー（セッション毎1回）
- **View files (*.erb)**: 10秒後にコミットリマインダー（セッション毎1回）

#### 4. フック設定
- `.claude/hooks.json`: work_completionフック設定
- `.claude/settings.json`: 作業完了自動化設定
- `package.json`: dev:commitコマンド定義
...
### 効果と目的
1. **コミット忘れ防止**: ファイル変更検知による自動リマインダー
2. **ワークフロー統一**: 対話式コミットによる品質向上
3. **効率化**: PR自動作成までの一連の流れをサポート
4. **品質保証**: コミットタイプとメッセージの標準化
